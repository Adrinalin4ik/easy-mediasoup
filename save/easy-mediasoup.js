// protooClient = require('protoo-client')
// mediasoupClient = require('mediasoup-client')
// Logger = require('./logger')
// cookiesManager = require('./cookies-manager')

import protooClient from 'protoo-client';
import * as mediasoupClient from 'mediasoup-client';
import Logger from './logger';
// // import { getProtooUrl } from './urlFactory';
import * as cookiesManager from './cookies-manager';
...
{
	constructor(
		{ roomId, peerName, displayName, useSimulcast, produce })
	{	
		console.log(logger)
		logger.debug(
			'constructor() [roomId:"%s", peerName:"%s", displayName:"%s"]',
			roomId, peerName, displayName);
		let hostname = "localhost"
		const protooUrl = `wss://${hostname}:3443/?peerName=${peerName}&roomId=${roomId}`;
	...
	_joinRoom({ displayName })
	{
		...
		this._room.join(this._peerName, { displayName})
			.then(() =>
			{
	...
	_handleConsumer(consumer)
	{
		...
		// 		.then((track) =>
		// 		{
		// 			// _dispatch(stateActions.setConsumerTrack(consumer.id, track));
		// 		})
		// 		.catch((error) =>
		// 		{
		// 			logger.error(
